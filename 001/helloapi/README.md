...
## Основные эндпоинты
- `GET http://localhost:8080/hello` – возвращает простой текстовый ответ.
- `GET http://localhost:8080/user` – возвращает JSON с информацией о пользователе (UUID и имя).
- `GET http://localhost:8080/health` – эндпоинт для проверки состояния сервиса, возвращающий JSON с полем "status" и текущим временем в формате RFC3339.
